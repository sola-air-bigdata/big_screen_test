...
class StationShowMmTableUpdate {
  ...
  def analyzeUpdateTable(jsonSql: JSONObject) {
    val jedis = jedisutil
    try {
      //     log.info("********StationShowMonthMemberTableUpdate处理json:" + jsonSql.toJSONString())
      ...
      for (m <- 0 to arrSize - 1) {
        ...
        while(itr.hasNext()){
          var s = itr.next()
          var key = "SMMT"+"_"+l_station_id+"_"+l_seller_id+"_"+d_month_time+"_"+l_member_id+"_"+s
      ...
      case ex: Exception => log.info("*****StationShowMonthMemberTableUpdate**redis处理异常****")
    } finally {
      if(jedis!=null){
        jedis.close()
      }
