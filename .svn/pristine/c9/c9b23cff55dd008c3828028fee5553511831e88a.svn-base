...
class AllDayMemberTableUpdate {
  ...
  def analyzeUpdateTable(jsonSql: JSONObject) {
    val jedis = jedisutil
    try {
      //     log.info("********AllDayMemberTableUpdate处理json:" + jsonSql.toJSONString())
      ...
      for (m <- 0 to arrSize - 1) {
        ...
        while(itr.hasNext()){
          var s = itr.next()
          var key = "DMEMT"+"_"+l_seller_id+"_"+d_day_time+"_"+l_member_id+"_"+s
      ...
      case ex: Exception => log.info("*****AllDayMemberTableUpdate**redis处理异常****")
       ex.printStackTrace()
    } finally {
      if(jedis!=null){
        jedis.close()
