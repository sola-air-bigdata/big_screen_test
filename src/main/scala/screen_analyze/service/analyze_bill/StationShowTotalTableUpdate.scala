...
class StationShowTotalTableUpdate {
  ...
  def analyzeUpdateTable(jsonSql: JSONObject) {
    val jedis = jedisutil
    try {
      //     log.info("********StationShowTotalTableUpdate处理json:" + jsonSql.toJSONString())
      ...
      for (m <- 0 to arrSize - 1) {
        ...
        while (itr.hasNext()) {
          var s = itr.next()
          var key = "STT" + "_" + l_station_id + "_" + l_seller_id + "_" +  s
      ...
      case ex: Exception => log.info("*****StationShowTotalTableUpdate**redis处理异常****")
    } finally {
      if (jedis != null) {
        jedis.close()
      }
