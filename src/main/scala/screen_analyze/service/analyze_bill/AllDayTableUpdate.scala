...
class AllDayTableUpdate {
  ...
  def analyzeUpdateTable(jsonSql: JSONObject) {
    val jedis = jedisutil
    try {
      //     log.info("********AllDayTableUpdate处理json:" + jsonSql.toJSONString())
      ...
      for (m <- 0 to arrSize - 1) {
        ...
        while (itr.hasNext()) {
          var s = itr.next()
          var key = "ADT" + "_" + l_seller_id + "_" + d_day_time + "_" + s
      ...
      case ex: Exception => log.info("*****AllDayTableUpdate**redis处理异常****")
    } finally {
      if (jedis != null) {
        jedis.close()
      }
