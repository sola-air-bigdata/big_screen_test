...
class AllMemberTableUpdate {
  ...
  def analyzeUpdateTable(jsonSql: JSONObject) {
    val jedis = jedisutil
    try {
      //     log.info("********AllMemberTableUpdate处理json:" + jsonSql.toJSONString())
      ...
      for (m <- 0 to arrSize - 1) {
        ...
        while (itr.hasNext()) {
          var s = itr.next()
          var key = "AMEMT" + "_" + l_member_id+"_"+s
      ...
      case ex: Exception => log.info("*****AllMemberTableUpdate**redis处理异常****")
    } finally {
      if (jedis != null) {
        jedis.close()
      }
