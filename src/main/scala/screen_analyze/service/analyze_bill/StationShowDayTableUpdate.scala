...
class StationShowDayTableUpdate {
  ...
  def analyzeUpdateTable(jsonSql: JSONObject) {
    val jedis = jedisutil
    try {
      //     log.info("********StationShowDayTabl   eUpdate处理json:" + jsonSql.toJSONString())
      ...
      for (m <- 0 to arrSize - 1) {
        ...
        val d_day_time = data.getString("d_day_time")
        val l_station_id = data.getLongValue("l_station_id")
        val keys = old.keySet()
        val itr = keys.iterator()
        while(itr.hasNext()){
          var s = itr.next()
          var key = "SDT"+"_"+l_station_id+"_"+l_seller_id+"_"+d_day_time+"_"+s
      ...
      case ex: Exception => log.info("*****StationShowDayTableUpdate**redis处理异常****")
    } finally {
      if(jedis!=null){
        jedis.close()
      }
