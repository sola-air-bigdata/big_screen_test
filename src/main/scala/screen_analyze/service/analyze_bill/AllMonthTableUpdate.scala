...
class AllMonthTableUpdate {
  ...
  def analyzeUpdateTable(jsonSql: JSONObject) {
    val jedis = jedisutil
    try {
      //     log.info("********AllMonthTableUpdate处理json:" + jsonSql.toJSONString())
      ...
      for (m <- 0 to arrSize - 1) {
        ...
        while(itr.hasNext()){
          var s = itr.next()
          var key = "AMT"+"_"+l_seller_id+"_"+d_month_time+"_"+s
      ...
      case ex: Exception => log.info("*****AllMonthTableUpdate**redis处理异常****")
    } finally {
      if(jedis!=null){
        jedis.close()
      }
